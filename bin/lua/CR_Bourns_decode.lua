...
local function Check_CR_Name(Name)  --FIXME допилить...
  ...
  Name_updated = Name_updated:gsub('[сС]', 'C')
  if( Name_updated ~= Name ) then
    E_message = 'В наименовании присутствует русская буква "С". Должна быть английская буква "C".\n'
    return false, E_message, Name_updated
  end

--  Name_updated:match('(P1%-12)%-([%d,]+)%-([%d,]+) ([кМ]*Ом)±([%d,]) %%[%-МЛМ1ТУ]*')
