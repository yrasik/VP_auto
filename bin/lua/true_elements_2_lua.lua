...
local function info()
  local info = 
[[
  Модуль для манипуляций с файлом 'true_elements.txt' (файл с одобренными нормоконтролем полными названиями элементов).
  Выполняет:
...
local function get_data_from_value(Value, Name, flog, line, state)
  ...
  if( Value ~= nil ) then
    ...
    for i = 1, #Name  do
      ...
      Name_ = Name[i].Name..' ' -- "ная сборка VS-6CWQ10FNTR-M3",
